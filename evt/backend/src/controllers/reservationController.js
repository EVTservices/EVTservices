...
const { Reservation, Route, Stop, Factory, Bus, User, BusSchedule } = require("../models");
const { Op } = require("sequelize");
const { sendLineMessage } = require("../utils/lineNotifier");
...
exports.createReservation = async (req, res) => {
  ...
  try {
    ...
    // ✅ Find the matching schedule for today and this bus
    const currentDay = new Date().getDay(); // Sunday = 0, Monday = 1, ...
    ...
    if (!schedule) return res.status(404).json({ error: "No matching bus schedule found." });
